  ...
  return (
    <div className="space-y-3">
      ...
      <div className="divide-y-2 divide-black/10 border-2 border-black/10 rounded-xl overflow-hidden bg-white">
        {markets.map((m) => {
          ...
          return (
            <button
              ...
              className={`w-full text-left px-4 py-3 flex items-center justify-between ${isSel ? "bg-black/5" : "bg-white"}`}
            >
              <div>
                <div className="font-bold text-[#0b1f3a]">{m.title}</div>
                <div className="text-xs text-[#0b1f3a] opacity-70 mt-0.5">
                  {m.isPrivate ? "Private" : "Public"} · Liquidity {m.liquidity.toLocaleString(undefined, { maximumFractionDigits: 2 })} SOL
                </div>
              </div>
              <div className="text-xs">
                <span className={`px-2 py-1 rounded-full border-2 ${isResolved ? "border-slate-700 text-slate-800" : inOpportunity ? "border-amber-700 text-amber-800" : "border-emerald-700 text-emerald-800"}`}>
                  {windowLabel}
                </span>
              </div>
            </button>
        ...
        {markets.length === 0 && (
          <div className="px-4 py-6 text-sm text-[#0b1f3a] opacity-70">No markets yet.</div>
        )}
      </div>
    </div>
  );
