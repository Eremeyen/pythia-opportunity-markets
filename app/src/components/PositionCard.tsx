...
import {
	getCurrentValueUsd,
	getNextPublicStartMs,
	isClaimEnabled,
	getClaimType,
} from '../config/mockPositions';
		...
		<div className="w-full border-4 border-black rounded-2xl p-4 bg-white flex flex-col gap-3">
			<div className="flex items-center justify-between">
				...
				>
					...
					<div>
						...
						<div className="text-sm md:text-base font-extrabold text-[#0b1f3a] opacity-80">
							{chipLabel(position.phase)}
						</div>
					</div>
				</Link>
				{renderClaimButton()}
			...
			<div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
				...
				<Stat
					label="Quantity"
					value={
						position.phase === 'private' ? 'Hidden' : position.quantity.toLocaleString()
					}
				/>
		...
		return (
			...
			>
				<span className="inline-flex items-center gap-2">
					{isClaiming ? <Spinner className="w-4 h-4" /> : null}
					<span>{label}</span>
				</span>
			</button>
		);
...
function Stat({ label, value }: { label: string; value: string }) {
	return (
		<div className="flex flex-col">
			<span className="text-xs uppercase tracking-wide text-[#0b1f3a] opacity-60">
				{label}
			</span>
