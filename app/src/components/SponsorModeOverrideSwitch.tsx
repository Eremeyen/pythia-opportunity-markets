...
export default function SponsorModeOverrideSwitch() {
	...
	const badge = useMemo(() => {
		...
		if (whitelisted)
			return (
				<span className={`${base} border-green-700 text-green-800 bg-green-50`}>
					Whitelisted
				</span>
			);
		return (
			<span className={`${base} border-amber-700 text-amber-800 bg-amber-50`}>
				Not whitelisted
			</span>
		);
	}, [whitelisted]);
	...
	return (
		<div className="flex items-center gap-3">
			...
			{overrideSource && (
				<span className="text-[10px] italic text-[#0b1f3a] opacity-60">
