...
export default function SponsorMarketDetails({
	...
	const inPublic =
		!isResolved && !inOpportunity && (market.resultsEndMs ? now < market.resultsEndMs : true);
	const windowLabel = isResolved
		? `Resolved ${market.resolution ?? ''}`
		: inOpportunity
			? 'Opportunity window'
			: 'Public window';
	const targetMs = inOpportunity
		? market.opportunityEndMs
		: (market.resultsEndMs ?? market.opportunityEndMs);
	...
	const showNextOpp =
		inPublic &&
		!!market.nextOpportunityStartMs &&
		now < (market.nextOpportunityStartMs as number);
	...
	return (
		<div className="space-y-4">
			<div className="border-2 border-black rounded-xl overflow-hidden bg-white">
				<div className="px-4 py-3 border-b-2 border-black bg-black text-white font-bold">
					Market
				</div>
				<div className="p-4 space-y-3">
					<div className="grid grid-cols-1 md:grid-cols-[1fr_520px] items-start gap-6">
						<div>
							...
							<div className="text-sm text-[#0b1f3a] opacity-80 mt-1">
								{market.description}
							</div>
							<div className="mt-2 text-xs text-[#0b1f3a] opacity-70">
								...
								SOL · {market.isPrivate ? 'Private' : 'Public'}
							</div>
						</div>
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
						...
						/>
						<Kpi
							label={showNextOpp ? 'Next opportunity in' : 'Resolution date'}
							value={
								showNextOpp
									? nextOppCountdown
									: market.resultsEndMs
										? new Date(market.resultsEndMs).toLocaleDateString()
										: '—'
							}
						/>
					</div>
...
function Kpi({ label, value }: { label: string; value: string }) {
	return (
		<div className="p-3 rounded-lg border-2 border-black/10 bg-white">
			<div className="text-[11px] uppercase tracking-wide text-[#0b1f3a] opacity-60">
				{label}
			</div>
