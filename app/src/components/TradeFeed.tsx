...
export default function TradeFeed({
	...
	return (
		<div className="border-2 border-black rounded-xl overflow-hidden bg-white">
			<div className="px-3 py-2 font-bold border-b-2 border-black bg-black text-white">
				Trades
			</div>
			<div className="max-h-64 overflow-auto divide-y-2 divide-black/10">
					...
					.map((t) => (
						...
						>
							<div className="flex items-center gap-3">
								<span
									className={`px-2 py-0.5 rounded-full border-2 ${
										t.side === 'BUY_YES'
								...
								<span className="text-[#0b1f3a] opacity-80">
									{t.size} @ {t.price.toFixed(1)}%
								</span>
							</div>
							<div className="text-right">
								<div className="text-xs text-[#0b1f3a] opacity-60">
				...
				{trades.length === 0 && (
					<div className="px-3 py-4 text-sm text-[#0b1f3a] opacity-70">
						No trades yet.
					</div>
				)}
			</div>
