  ...
  const ensureMinCounts = (publicMin = 12, privateMin = 12) => {
    ...
    const makeClone = (m: SponsorMarket, i: number): SponsorMarket => {
      ...
      return {
        ...
        id: `${m.id}-alt-${i}`,
        title: newTitle,
        company: { ...(m.company ?? { id: `c-${m.id}` }), id: `${m.company?.id ?? `c-${m.id}`}-alt-${i}` , name: newCompanyName, logoUrl: newLogo },
        priceSeries: (m.priceSeries ?? []).map((v) => Math.max(0, v + (Math.random() - 0.5) * 2)),
        attentionScore: Math.max(1, Math.min(100, (m.attentionScore ?? 50) + Math.round((Math.random() - 0.5) * 10))),
      };
    };
...
export function updatePersistedMarketResolution(marketId: string, resolution: "YES" | "NO"): void {
  try {
    if (typeof window === "undefined") return;
    const current = readPersistedCreated();
    const next = current.map((x) => (x.id === marketId ? { ...x, status: "resolved", resolution } : x));
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
  } catch {}
}
