...
export default function MarketDetails() {
		...
		window.setTimeout(() => {
			setPositions((prev) => {
				...
				if (existingIndex >= 0) {
					...
					const newAvgEntry =
						newQuantity > 0 ? newInvestedUsd / newQuantity : existing.avgEntryPrice;
	...
	return (
		<div className="w-full">
			...
			<div
				className={`${
					isPrivateWindow ? '' : '-mx-0.5 md:-mx-1'
				} grid grid-cols-1 md:grid-cols-[1fr_420px] gap-8 items-start`}
			>
				<div className="space-y-8">
					...
					<section className={isPrivateWindow ? 'py-8' : ''}>
						...
						<div
							className={`${
								isPrivateWindow ? 'mt-4' : 'mt-2'
							} border-4 border-black rounded-2xl p-4 bg-white`}
						>
							...
							<dl className="mt-4 grid grid-cols-1 gap-3 text-sm md:text-base text-[#0b1f3a]">
								...
								{market.company.totalRaisedUsd != null && (
									<div className="flex items-baseline justify-between">
										...
										<dd className="font-extrabold tabular-nums">
											{formatUsd(market.company.totalRaisedUsd)}
										</dd>
									</div>
								)}
								...
								{market.company.notableInvestors &&
									market.company.notableInvestors.length > 0 && (
										<div className="flex items-baseline justify-between">
											<dt className="font-bold opacity-70 uppercase tracking-wide text-xs md:text-sm">
												Investors
											</dt>
				...
				<div className="space-y-6">
					...
					/>
					{isPrivateWindow && <div className="h-10 md:h-16" />}
					<section className="border-4 border-black rounded-2xl bg-white p-5">
						{!sponsorReady ? (
							...
							<>
								...
								<dl className="mt-4 grid grid-cols-1 gap-3 text-sm md:text-base text-[#0b1f3a]">
									{market.sponsor.focusSectors &&
										market.sponsor.focusSectors.length > 0 && (
											<div className="flex items-baseline justify-between">
												<dt className="font-bold opacity-70 uppercase tracking-wide text-xs md:text-sm">
													Focus
												</dt>
									...
									{market.sponsor.stageFocus &&
										market.sponsor.stageFocus.length > 0 && (
											<div className="flex items-baseline justify-between">
												<dt className="font-bold opacity-70 uppercase tracking-wide text-xs md:text-sm">
													Stages
												</dt>
