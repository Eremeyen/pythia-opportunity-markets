...
export default function ProfilePage() {
	...
	const onClaim = (id: string) => {
		...
		setTimeout(() => {
			setPositions((prev) =>
				prev.map((p) =>
					p.id === id ? { ...p, claimed: true, hasPendingClaim: false } : p,
				),
			);
		}, 1400);
	};
		...
		<div className="max-w-4xl w-full mx-auto">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
				<div className="border-4 border-black rounded-2xl bg-white p-4 flex flex-col gap-2">
					...
					<div className="text-base md:text-lg font-extrabold text-[#0b1f3a] font-mono">
						{short}
					</div>
					<div className="text-xs uppercase tracking-wide text-[#0b1f3a] opacity-60 mt-2">
						Portfolio Value
					</div>
					<div className="text-3xl md:text-4xl font-extrabold text-[#0b1f3a]">
						{fmtUsd(balanceUsd)}
					</div>
				</div>
					...
					<div>
						...
						<div
							className={`text-3xl md:text-4xl font-extrabold ${
								pnlUsd >= 0 ? 'text-green-700' : 'text-red-700'
							}`}
						>
							{fmtSignedUsd(pnlUsd)}
						</div>
