...
export default function TrendingMarketsPage({ scope = "trending" as "trending" | "public" | "private" }: { scope?: "trending" | "public" | "private" }) {
  ...
  const publicItems = publicBase.filter(
    (m) => m.status !== "resolved" && now >= m.opportunityEndMs && (m.resultsEndMs == null || now < m.resultsEndMs)
  );
  const privateItems = privateBase.filter((m) => m.status !== "resolved" && now < m.opportunityEndMs);
      ...
      {showPrivate && (scope === "trending" ? (
        ...
        <section aria-label="Private markets">
          <div className="mb-3 md:mb-4">
            <h2 className="text-xl md:text-2xl font-extrabold text-[#0b1f3a]">Private markets</h2>
          </div>
